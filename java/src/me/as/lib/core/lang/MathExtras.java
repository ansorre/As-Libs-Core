...
import static me.as.lib.core.lang.RandomExtras.random;
...
/*
 ...
 public static void main(String args[])
 {
  ...
  for (t=0;t<100;t++)
  {
   String f=MathUtil.truncate(v, 1)+";"+MathUtil.truncate(MathUtil.reduce(v, 0, 1), 1);
   f=replace(f, ".", ",");
   System.out.println(f);

//   System.out.println("v: "+MathUtil.truncate(v, 1)+" --> "+MathUtil.truncate(MathUtil.reduce(v, 0, 1), 1));

   v=MathUtil.truncate(v+0.1, 12);
  }
 }
...
/*
 ...
 public static void main(String args[])
 {
  ...
  StringUtil.systemOut(serie);

  System.out.println("normalized: ");
  StringUtil.systemOut(normalize(serie));
 }
*/
...
/*
 ...
 public static double getPercentSimilarity(double v1, double v2, double range)
 {
  ...
  if (res==0)
  {
   res=MathUtil.expressedAsPercentOf(v1, v2);

   if (MathUtil.isOutside(res, -range, range)) res=0;
   else
   {
...
//  return (smaller<than && (100.0-expressedAsPercentOf(smaller, than))>=ofAtLeastThisPerc);
 ...
 private static final String toDouble_err = "Could not convert to double the value passed to MathUtil.toDouble(...) which class is '";
